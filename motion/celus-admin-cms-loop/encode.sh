...
ffmpeg -y -ss 00:00:00.500 -i "${S}.mp4" -vframes 1 "${OUT_DIR}/${NAME}.poster-source.png"
...
ffmpeg -y -i "${S}.mp4" -vf "$SCALE" -an -c:v libvpx-vp9 -pix_fmt yuv420p -profile:v 0 \
  -crf 36 -b:v 0 -row-mt 1 -deadline good -cpu-used 2 $TAG -pass 1 -f null /dev/null
ffmpeg -y -i "${S}.mp4" -vf "$SCALE" -an -c:v libvpx-vp9 -pix_fmt yuv420p -profile:v 0 \
  -crf 36 -b:v 0 -row-mt 1 -deadline good -cpu-used 2 $TAG -pass 2 "${OUT_DIR}/${NAME}.webm"
