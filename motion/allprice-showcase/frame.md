...
description: >
  ...
  accent, the three-step gray text ladder, Lato (display/numerals/chrome) + Lato (body),
...
colors:
  ...
  text-muted: "#384257"
  text-light: "#9A9A9A"
  accent-light: "rgba(35, 77, 215, 0.08)"
  accent-medium: "rgba(35, 77, 215, 0.15)"
  border: "rgba(35, 77, 215, 0.2)"
  card-bg: "rgba(35, 77, 215, 0.04)"
  positive: "#059669"
  negative: "#dc2626"
...
typography:
  # — reading ramp (Lato body + Lato chrome) —
  body:    { fontFamily: "Lato", cqw: 0.85, weight: 400, lineHeight: 1.6, color: "text-muted" }
  h4-eyebrow:{ fontFamily: "Lato", cqw: 0.8, weight: 700, tracking: "0.08em", upper: true, color: "primary" }
  tag:     { fontFamily: "Lato", px: 12, weight: 500, color: "primary" }
  counter: { fontFamily: "Lato", px: 13, weight: 500, tracking: "0.05em", color: "text-muted" }
  # — display / numerical ramp (Lato, near-black headings / cobalt numerals) —
  h3:      { fontFamily: "Lato", cqw: 1.25, weight: 500, lineHeight: 1.3, tracking: "-0.02em", color: "text" }
  stat-num:{ fontFamily: "Lato", cqw: 1.9, weight: 700, lineHeight: 1.0, color: "primary" }
  blockquote:{ fontFamily: "Lato", cqw: 2.4, weight: 500, lineHeight: 1.35, color: "text" }
  h2:      { fontFamily: "Lato", cqw: 2.6, weight: 700, lineHeight: 1.1, tracking: "-0.02em", color: "text" }
  metric-value:{ fontFamily: "Lato", cqw: 3.0, weight: 700, lineHeight: 1.0, color: "primary" }
  h1:      { fontFamily: "Lato", cqw: 4.2, weight: 700, lineHeight: 1.08, tracking: "-0.02em", color: "text" }
  quote-mark:{ fontFamily: "Lato", cqw: 8.0, weight: 700, lineHeight: 0.5, color: "primary", opacity: 0.15 }
...
Capture-remap correction: the generator assigned the bright sky-blue icon color
to body text. `text-muted` is the captured AllPrice ink `#384257` so small captions
remain readable on white. No source UI pixels are recolored. The generated
`text-light` token is decorative only; never use it for load-bearing captions.
...
- **Fonts** — already set to **Lato** (display) / **Lato** (body); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{400, 500, 700, 800}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
The voice is two faces in fixed roles: **Lato** (display, every numeral, all chrome —
eyebrows uppercase 0.08em) and **Lato** (body, muted gray, line 1.6). Headlines are near-black;
...
- **Lato** (display/numerals/chrome) + **Lato** (body) — near-black headlines, cobalt numerals.
...
Two ramps. The **reading ramp** (Lato body 0.85cqw muted; Lato eyebrow uppercase 0.08em
cobalt) carries copy + chrome; the **display/numerical ramp** (Lato `h3` 1.25cqw → `h1`
...
- **Headlines near-black, −0.02em**; **eyebrows cobalt, uppercase, 0.08em**; **numerals cobalt 600–700**; **body Lato 400 muted, line 1.6**. No italic, no uppercase body, no cobalt headline.
...
**Composes** accent-line, meta, h1, body sub. **Focal** a 2-line Lato `h1` near-black, left,
...
**Focal** a row of tinted cards — cobalt `metric-value` + Lato label + muted desc + optional
...
blockquote, cite. **Focal** a Lato `blockquote` near-black under a 15%-opacity cobalt
...
Lato body column beside a cobalt-8% highlight block (4px cobalt left rule) carrying an inline pull
...
a Lato `h1` near-black, centered, with the one solid cobalt `cta-button` pill below. **Accent**
...
- Use **tinted cards** (4% fill, 20% border, 10–14px radius, no shadow); body Lato 400 muted, line 1.6.
...
- **Type** — Lato headings −0.02em near-black, cobalt eyebrows 0.08em + numerals; Lato body muted line 1.6; ≥1.4cqw floor.
...
- **Lato + Lato via Google Fonts.** CJK pairing (Noto Sans SC 700 display / Noto Serif SC 400 body) carries over; the eyebrow's uppercase+tracking signal weakens in CJK — pair it with the accent-line.
