...
NAME="psiativa-funnel-calculator-loop"
...
echo "=== 1. Extracting poster frame at 2.0s (settled calculator) ==="
ffmpeg -y -ss 00:00:02.000 -i "${S}.mp4" -vframes 1 "${OUT_DIR}/${NAME}.poster-source.png"
ffmpeg -y -i "${OUT_DIR}/${NAME}.poster-source.png" -vf "$SCALE" -c:v libwebp -quality 84 -preset picture "${OUT_DIR}/${NAME}.webp"
ffmpeg -y -i "${OUT_DIR}/${NAME}.poster-source.png" -vf "$SCALE" -q:v 3 "${OUT_DIR}/${NAME}.jpg"
...
# Clean up lossless reference and temporary poster
