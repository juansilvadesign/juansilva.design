...
| **D** | Port the v2 pages to Astro | ✅ | 14 EN/PT static pages, shared SEO layout, zero emitted JS — 08-05 |
| **E** | vCard | ⬜ | C ✅; independent and ready |
| **F** | Master-portfolio evidence store | ⬜ | C ✅; independent and ready |
| **G** | Contact form on Cloudflare Pages Functions | 🟢 | Active. Local implementation built; Cloudflare config + real inbox delivery remain |
| **H** | Cutover to `juansilva.design` | 🔒 | Gated on the domain purchase — deliberately deferred |

Critical path is now **G**, actively in progress. **E** and **F** can parallel G now that C and D have
landed. **G** closes the inbound loop. **H** waits on a purchase decision.
...
Shipped 2026-08-05: all seven routes render EN and PT at build time (14 pages),
the shared layout derives canonical/OG/hreflang/favicon metadata from `site` and
locale, project cards read the typed collection, and no JavaScript is emitted.
Native links, form validation, and `<details>` preserve the baseline without an
island. The superseded Next/Tailwind UI and broken GitHub Pages workflow were
removed after the Astro build passed.
...
This is the destination for the NFC/QR link and the prerequisite for closing the
*"tracked short link for QR/NFC: not set up yet"* item that has been open in
`MEMORY.md` since June.

Implementation completed locally 2026-08-06: the shared typed contact record now
drives both localized card routes, the existing site-wide social/mail constants,
and a generated vCard 3.0 endpoint. The build emits `/card/`, `/pt/card/`, and
...
Implementation note, 2026-08-05: Pages currently rejects the native Workers
Email and RateLimit bindings. The port therefore keeps the existing Gmail
account over outbound SMTP/TLS on port 465 and implements the existing 5 / 15
minute quota with a KV counter keyed by an HMAC of the visitor IP. Turnstile is
the only justified JavaScript exception to D: a hosted widget script is loaded on
the contact route, while form submission and localized feedback remain native
HTML with no island or custom browser bundle. The endpoint validates Turnstile's
success, action, and hostname server-side and never emits CORS headers.
